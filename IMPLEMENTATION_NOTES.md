...
Omusubiのformat実装は、C++23の`std::format`と`std::basic_format_string`をC++14で再現することを目指しています。
...
C++14では、プレースホルダー数と引数数の完全な一致チェックをコンパイル時に行うことは技術的に困難。
...
C++14のconstexpr関数は複雑なロジックを含められるが、例外は投げられない。
...
| 機能 | std::format (C++23) | omusubi::format (C++14) |
|------|---------------------|-------------------------|
| 基本フォーマット | ✅ | ✅ |
